...
from agents import CentralizedCritic, SharedAgentActor
...
class MAPPO:
    ...
    def act(self, obs: np.ndarray):
        ...
        state = obs_t.flatten().unsqueeze(0)
    ...
    def value(self, obs: np.ndarray) -> float:
        state = torch.tensor(obs.flatten(), dtype=torch.float32, device=self.device).unsqueeze(0)
    ...
    def update(self, batch: RolloutBatch):
        ...
        flat_states = batch.states[:, None, :].repeat(1, n, 1).reshape(t * n, -1)
        ...
        for _ in range(self.cfg.ppo_epochs):
            for obs, actions, old_log_probs, advantages, states, returns in loader:
