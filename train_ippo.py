...
from ippo import IPPO, RolloutBuffer
...
def train(args):
    ...
    algo = IPPO(env.obs_dim, env.global_state_dim, env.action_dim, cfg.train, device)
    history = []
    ...
    for episode in range(1, cfg.train.episodes + 1):
        ...
        if episode % cfg.train.checkpoint_every == 0:
            algo.save(cfg.paths.checkpoints / f"ippo_ep{episode}.pt")

    final_path = cfg.paths.checkpoints / "ippo_final.pt"
